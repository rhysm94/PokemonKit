...
//	let randomSource: GKRandomDistribution
//	
//	init(randomSource: GKRandomDistribution) {
...
//	}
	...
	var attackBonuses: [String: Attack.BonusEffect] = [
		...
		"Confuse Ray": .singleTarget({
			let diceRoll = Random.shared.confusion()//GKRandomDistribution(forDieWithSideCount: 4).nextInt()
			$0.volatileStatus.insert(.confused(diceRoll))
			print("\($0.nickname) became confused for \(diceRoll) turns!")
		}),
		"Dark Pulse": .singleTarget({
			let diceRoll = Random.shared.d5Roll()//GKRandomDistribution(forDieWithSideCount: 5).nextInt()
			if diceRoll == 1 {
		...
		"Extrasensory": .singleTarget({
			let diceRoll = Random.shared.d10Roll()//GKRandomDistribution(forDieWithSideCount: 10).nextInt()
			if diceRoll == 1 {
				$0.volatileStatus.insert(.flinch)
		...
		"Thunderbolt": .singleTarget({
			let diceRoll = Random.shared.d6Roll()//GKRandomDistribution.d6().nextInt()
			if diceRoll == 1 && $0.status == .healthy {
				$0.status = .paralysed
			}
		})
